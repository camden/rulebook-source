...
const main = () => {
  ...
  app.use(
    '/api',
    cacheClient({ match: '/config', expiryDays: 60 }),
    cacheClient({ match: '/pages', expiryDays: 60 }),
    cacheClient({ match: '/rulebooks', expiryDays: 30 }),
    cacheClient({ match: '/assets', expiryDays: 60 }),
    apiRouter
  );
